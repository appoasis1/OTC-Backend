...
export const sendToApproverEmail = async (to: string, requisition_number: string,total: number, originator_name: string, originator_surname ) => {
    let transporter = nodemailer.createTransport({
      ...
      let info = await transporter.sendMail({
        ...
        text: `Your have Requisition ${requisition_number} totalling ${formatCurrency(total)} from  ${originator_name} ${originator_surname} awaiting your approval`, // plain text body
        html: `<b>Your have Requisition ${requisition_number} totalling ${formatCurrency(total)} from  ${originator_name} ${originator_surname} awaiting your approval`, // html body
      });
}
...
export const finalNotification = async (to: string, requisition_number: string,total: number) => {
    let transporter = nodemailer.createTransport({
      ...
      let info = await transporter.sendMail({
        ...
        to: to, // list of receivers
        subject: "NOTIFICATION OF REQUISITION MOVEMENT", // Subject line
        text: `Your Requisition ${requisition_number} totalling ${formatCurrency(total)} is now awaiting Purchase Order Creation`, // plain text body
        html: `<b>Your Requisition ${requisition_number} totalling ${formatCurrency(total)} is now awaiting Purchase Order Creation`, // html body
      });
}
