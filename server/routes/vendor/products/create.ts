...
export default defineEventHandler(async (event) => {
  const { name, price, status, code, quantity, description, manufacturer, image, vendor_name } = await readBody(event);
  ...
  try {
    ...
    const filename = `${Date.now()}-${Math.floor(Math.random() * 10000)}.jpg`; // Assuming JPEG format for the image
    ...
    const createProduct = await prisma.product.create({
      data: {
        ...
        vendor_id: defaultVendorId, // Set the default vendor ID for the product
      },
    });
